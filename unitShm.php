...
$shm = new ShmQueue();
$shm->addQueue('processCore_web','w');
$shm->addQueue('processCore_work','r');
process(function () use ($shm){
    $shm->bindQueue('processCore_web','w');
    $shm->bindQueue('processCore_work','r');
    while (1){
        $shm->push('http web data','processCore_web','w');
        sleep(3);
        while ($data = $shm->pop('processCore_work','r')){
            echo $data.'____main'.PHP_EOL;
        }
    }
});
process(function () use ($shm){
    ...
    $shm->bindQueue('processCore_work','r');
    while (1){
        $shm->push('processCore_work','r');
        while ($data = $shm->pop( 'processCore_web','w')){
            echo $data.'______work'.PHP_EOL;
        }
        sleep(3);
    }
});
...
function process(callable $callback = null, callable $main = null)
{
    $pid = pcntl_fork();
    if ($pid == 0) {
